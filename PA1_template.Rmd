...
---
This file is created for submission of assignment for Reproducible Research, a Coursera Course.

##Preprosessing Data
```{r}
setwd("D:/Data_Science/Reproducible_Research/")
list.files()
activity <- read.csv('activity.csv')
```

Create a date.time column that combines the date and interval columns.
Also, For analyzing the means at the different times of day, it will also be convenient to have a time column. To do this, I convert all of the dates to be for today. since we only care about the time for that column, it will help us with the analysis.
```{r}
